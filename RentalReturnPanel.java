...
public class RentalReturnPanel extends BasePanel {
    ...
    private void processReturn(Connection conn, int rentalId, String bookTitle, String bookAuthor, String dueDate, double lateFee) throws SQLException {
        // Update rental record with return date, status, and payment status
        try (PreparedStatement pstmt = conn.prepareStatement(
                "UPDATE rentals SET return_date = datetime('now'), status = 'Returned', payment_status = 'pending' WHERE id = ?")) {
            pstmt.setInt(1, rentalId);
            pstmt.executeUpdate();
            System.out.println("Debug - Updated rental record with pending payment status");
        }
