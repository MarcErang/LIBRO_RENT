...
public class AdminDashboard extends JFrame {
    ...
    private void updateBookStats(JLabel totalBooksLabel, JLabel totalCopiesLabel, JLabel totalAvailableLabel) {
        try (Connection conn = dbManager.getConnection()) {
            ...
            try (Statement stmt = conn.createStatement();
                 ResultSet rs = stmt.executeQuery("""
                    SELECT COUNT(*) as overdue_count FROM rentals 
                    WHERE return_date IS NULL 
    ...
    private JPanel createRentalManagementPanel() {
        ...
        saveChangesButton.setBackground(new Color(52, 152, 219)); // Blue
        ...
        saveChangesButton.setFont(new Font("Arial", Font.BOLD, 12));
        ...
        saveChangesButton.addActionListener(e -> {
            int choice = JOptionPane.showConfirmDialog(this,
                "Are you sure you want to save all payment status changes?",
            ...
            if (choice == JOptionPane.YES_OPTION) {
                try (Connection conn = dbManager.getConnection()) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < rentalsModel.getRowCount(); i++) {
                            int rentalId = (int) rentalsModel.getValueAt(i, 0);
                            String paymentStatus = ((String) rentalsModel.getValueAt(i, 7)).toLowerCase();
                        ...
                        conn.commit();
                        JOptionPane.showMessageDialog(this,
                            "All payment status changes have been saved successfully!",
                            "Success",
                            JOptionPane.INFORMATION_MESSAGE);
                    ...
                    ex.printStackTrace();
                    JOptionPane.showMessageDialog(this,
                        "Error saving payment status changes: " + ex.getMessage(),
                        "Error",
                        JOptionPane.ERROR_MESSAGE);
                }
            }
        ...
        searchPanel.add(Box.createHorizontalStrut(20)); // Add some spacing
        searchPanel.add(saveChangesButton);
    ...
    private void showPaymentStatusDialog(int row) {
        ...
        String currentStatus = ((String) rentalsModel.getValueAt(row, 7)).toLowerCase(); // Normalize to lowercase
        ...
        saveButton.addActionListener(e -> {
            String newStatus = ((String) statusCombo.getSelectedItem()).toLowerCase(); // Normalize to lowercase
                    ...
                    try {
                        ...
                        try (PreparedStatement pstmt = conn.prepareStatement(
                            ...
                            if (rowsAffected > 0) {
                                ...
                                // Refresh the rentals table to ensure data consistency
                                loadRentals();
                                
                                dialog.dispose();
                                JOptionPane.showMessageDialog(this,
                                    "Payment status has been successfully changed to '" + newStatus + "'!",
                                    "Success",
                                    JOptionPane.INFORMATION_MESSAGE);
                            } else {
                                conn.rollback();
                                ...
                                JOptionPane.showMessageDialog(dialog,
                                    "Failed to update payment status. Rental record not found.",
                                    "Error",
                                    JOptionPane.ERROR_MESSAGE);
                            }
                        }
                        ...
                        // Rollback transaction on error
                        conn.rollback();
                        System.out.println("Error occurred, rolling back transaction: " + ex.getMessage());
                        throw ex;
                    } finally {
                        // Reset auto-commit
                        conn.setAutoCommit(true);
            ...
            try (PreparedStatement pstmt = conn.prepareStatement("""
                ...
                WHERE LOWER(b.title) LIKE LOWER(?)
                   ...
                   OR LOWER(CASE 
                           ...
                           ELSE 'Active'
                        END) LIKE LOWER(?)
                   OR LOWER(COALESCE(r.payment_status, 'pending')) LIKE LOWER(?)
             ...
             """)) {
                ...
                while (rs.next()) {
                    ...
                    System.out.println("Rental Date: " + rentalDateStr);
